...
class MythosConfig:
    ...
    dropout: float = 0.0
    # [2026] Sliding Window size for KV Cache (0 = disabled)
    sliding_window_size: int = 1024


# ---------------------------------------------------------------------------
# GRPO (Group Relative Policy Optimization) - 2026 Industry Standard
# ---------------------------------------------------------------------------
...
class GRPOLoss(nn.Module):
    ...
    Used for reasoning alignment without a separate critic model.
    Calculates rewards relative to the group mean for a set of samples.
    """
    def __init__(self, eps: float = 0.2):
        super().__init__()
        self.eps = eps

    def forward(self, logits, actions, advantages):
        """
        Args:
            logits: model outputs (B, T, V)
            actions: sampled token ids (B, T)
            advantages: relative rewards (B,) calculated across the group
        """
        log_probs = F.log_softmax(logits, dim=-1)
        action_log_probs = log_probs.gather(-1, actions.unsqueeze(-1)).squeeze(-1)
        
        # Policy gradient loss with relative advantages
        loss = -(action_log_probs * advantages.unsqueeze(-1)).mean()
        return loss
...
class GQAttention(nn.Module):
    ...
    def __init__(self, cfg: MythosConfig):
        ...
        self.sliding_window = cfg.sliding_window_size
    ...
    def forward(
        ...
        if kv_cache is not None:
            ...
            # [2026] Sliding Window KV Cache
            if self.sliding_window > 0 and k.shape[1] > self.sliding_window:
...
class MLAttention(nn.Module):
    ...
    def forward(
        ...
        if kv_cache is not None:
            ...
            # [2026] Sliding Window KV Cache for MLA
            if self.sliding_window > 0 and c_kv.shape[1] > self.sliding_window:
                c_kv = c_kv[:, -self.sliding_window:]
                k_rope = k_rope[:, -self.sliding_window:]
